...
await p.goto('http://localhost:8731/museum/', { waitUntil:'networkidle' });
...
const afterType = await p.textContent('#count');

// test search
await p.fill('#q', '爱');
await p.waitForTimeout(400);
const afterSearch = await p.textContent('#count');
await p.screenshot({ path:'/tmp/museum_filtered.png' });

console.log(JSON.stringify({ cells, count, imgStats, detailOpen, dName, dImgOk, afterType, afterSearch, errs }, null, 2));
await b.close();
